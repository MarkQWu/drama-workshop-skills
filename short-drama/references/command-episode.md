...
- **对白段落边界：** 剧本正文内，每个对白/OS/VO cue 独占一个源 Markdown 段落，前后空行隔开；同一段落里不得出现第二个 `**角色名**`。连续同一角色发言可以合成一条台词或拆成多个独立段；同声、重叠、群众反应用 `△` 动作段说明，或使用 `**众人**：` 聚合 cue。括号动作不逐句复制，状态延续改用一条 `△` 动作段承载。
