...
**对白段落边界（强制扣分/阻断）：** 每个对白回合必须独立成段，前后用空行隔开；同一个 Markdown 段落里不得出现第二个 `**角色名**` cue。连续同一角色说多句，可以合成同一条台词或拆成多个独立对白段，但不得写成 `**角色**：A。 **角色**：B。 **角色**：C。` 粘连段。括号动作若连续重复，应改为一条 `△` 动作段或只保留第一次，不逐句复制。
...
4. **对白段落边界**：每个 `**角色名**：` / `**角色名**（语气）：` / OS / VO cue 独占一个 Markdown 段落；用空行隔开，不能只用软换行，更不能把多个 cue 压进一段。
