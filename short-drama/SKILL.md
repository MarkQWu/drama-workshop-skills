...
3. 若 current_version ≠ last_shown：
   ...
   - 若 `{skill目录}/WHATSNEW.md` 不存在，或首个版本号与 current_version 不一致 → 在命令输出最前面展示内置更新提醒：”v1.34.0 更新：新增 /配置数据 命令，在对话里完成网文大数据 Key 配置，无需命令行；/市场、/选题会、/策划 可选调用实时榜单数据。另：剧本正文破折号完全禁用，`——`、`—`、`--` 任意出现均标【严重】。”
